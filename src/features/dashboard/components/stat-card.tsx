...
export function StatCard({ label, value, delta, trend, icon: Icon }: Props) {
  ...
  return (
    <div className="m-panel m-panel-shine p-6">
      <div className="flex items-start justify-between gap-3">
        <span className="meta-label text-text-mid">{label}</span>
        <span className="flex size-8 items-center justify-center rounded-md bg-brand-soft text-brand">
          <Icon aria-hidden="true" className="size-4" />
        </span>
      </div>

      <p className="display-title tnum mt-5 text-[2rem] leading-none text-text-hi">{value}</p>
